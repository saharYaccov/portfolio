...
class HUD:
    ...
    def _draw_panel(self, surface, gs):
        ...
        # תצוגת אויבים יורים (קטן)
        shooters_alive = gs.get("shooters_alive", 0)
        if shooters_alive > 0:
            # יצירת פונט קטן במיוחד
            font_tiny = pygame.font.SysFont("consolas", 14, bold=True)
            sh_col = C_RED if shooters_alive <= 2 else (255, 160, 0)  # כתום/אדום
            sh_lbl = font_tiny.render(f"Shooters:{shooters_alive}", True, sh_col)
            surface.blit(sh_lbl,(x,y2+4)); x += sh_lbl.get_width()+PAD; self._vd(surface,x,y2,20)
