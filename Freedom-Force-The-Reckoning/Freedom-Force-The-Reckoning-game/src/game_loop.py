...
from hud            import HUD
from enemy          import record_player, EnemyGroup, get_report_text
...
class Game:
    def __init__(self, base_dir=".", difficulty: str = "easy"):
        ...
        # difficulty + shared background
        self.difficulty = difficulty
        self._stage_bg  = self._load_stage_background(base_dir)
    ...
    def _draw_start_screen(self):
        ...
        # background image from assets (if available), otherwise gradient
        if self._stage_bg is not None:
            # full background with a light dark overlay for readability
            self.screen.blit(self._stage_bg, (0, 0))
    ...
    def _init_stage(self):
        ...
        if self.level.is_boss_stage:
            ...
            # Add shooting soldiers based on difficulty
            self._spawn_boss_soldiers(stage)
            self.state = "boss"
    ...
    def _spawn_boss_soldiers(self, stage):
        """Add shooting soldiers in boss stage based on difficulty level."""
        # חישוב מספר האויבים לפי רמת הקושי
        if self.difficulty == "easy":
            num_soldiers = 4
        elif self.difficulty in ("middle", "medium"):
            num_soldiers = 8
        elif self.difficulty == "hard":
            num_soldiers = 12
        else:
            num_soldiers = 5  # ברירת מחדל

        print(f"[DEBUG] Spawning {num_soldiers} boss soldiers (difficulty: {self.difficulty})")

        w = stage.width

        # רשימת גבהים אפשריים (בסיבוב)
        possible_heights = [2 * TILE, 4 * TILE, 6 * TILE, 8 * TILE, 10 * TILE]

        for i in range(num_soldiers):
            # Distribute evenly across the stage
            frac = (i + 1) / (num_soldiers + 1)
            ex   = int(w * frac)

            # בחירת גובה בסיבוב כדי למנוע התנגשויות
            ey = possible_heights[i % len(possible_heights)]
        ...
        print(f"[DEBUG] Successfully spawned {num_soldiers} boss soldiers")
        print(f"[DEBUG] Total boss soldiers in group: {len(self.extra_enemies)}")

    def _spawn_extra_enemies(self, stage):
        """מוסיף אויבים יורים לפי רמת הקושי."""
        # חישוב מספר האויבים לפי רמת הקושי
        if self.difficulty == "easy":
            num_enemies = 4
        elif self.difficulty in ("middle", "medium"):
            ...
            num_enemies = 5  # ברירת מחדל

        print(f"[DEBUG] Spawning {num_enemies} extra enemies (difficulty: {self.difficulty})")
        w = stage.width
        ...
        print(f"[DEBUG] Successfully spawned {num_enemies} extra enemies")
        print(f"[DEBUG] Total extra_enemies in group: {len(self.extra_enemies)}")
    ...
    def _update_boss(self, dt, stage, p):
        ...
        # Extra shots vs player
        for shot in list(self.extra_shots):
            shot.update(dt=dt, world_width=stage.width, platforms=stage.platforms)
            if shot.alive() and shot.rect.colliderect(p.rect) and not p.is_immune:
                shot.kill(); self._register_player_hit()

        # Player laser vs Extra enemies
        for laser in list(self.laser_group):
            for ee in list(self.extra_enemies):
                if ee.alive_flag and laser.rect.colliderect(ee.rect):
                    laser.kill()
    ...
    def _draw(self, dt):
        ...
        self.hud.draw(self.screen, dt, {
            ...
            # ── אויבים יורים ───────────────────────────────────────────────
            "shooters_alive":      len([e for e in self.extra_enemies if e.alive_flag]),
            # ── מקש אחרון ─────────────────────────────────────────────────
    ...
    def _draw_background(self):
        """
        Level background: keep existing gradient and overlay the game background
        image with very low opacity.
        """
        ...
        # overlay the shared background image with very low opacity
        if self._stage_bg is not None:
            bg = self._stage_bg.copy()
            bg.set_alpha(40)  # very subtle
    ...
    @staticmethod
    def _load_stage_background(base_dir: str):
        """
        Load a full-screen background image from the assets folder, if present.
        Used both for the game start screen and level backgrounds.
        """
        try:
            ...
            bg = pygame.transform.scale(img, (SCREEN_W, SCREEN_H))
            print(f"[game] background image loaded from {img_path}")
            return bg
        except Exception as e:
            print(f"[game] failed to load background image: {e}")
            return None
