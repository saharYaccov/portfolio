...
class TitleScreen:
    ...
    def _build_difficulty_rects(self):
        bw, bh = 180, 54
        cx = self.W // 2
        base_y = self.H // 2 + 180 + 20  # Moved down from 150 to 180 (30px lower)
        gap = 2  # 2px padding between buttons as requested
        self.rect_easy   = pygame.Rect(cx - bw*3//2 - gap, base_y, bw, bh)
        self.rect_mid    = pygame.Rect(cx - bw//2,          base_y, bw, bh)
        self.rect_hard   = pygame.Rect(cx + bw//2 + gap,    base_y, bw, bh)
    ...
    def _draw_hints(self):
        ...
        # Single large semi-transparent panel covering both hints and difficulty buttons
        # Height calculation: hints (175) + difficulty section (90) + spacing
        panel_height = 280

        # צור Surface שקוף
        panel = pygame.Surface((550, panel_height - 30), pygame.SRCALPHA)

        # צייר מלבן עם פינות מעוגלות
        pygame.draw.rect(
            panel,
            C_PANEL_BG,  # צבע הרקע עם השקיפות
            panel.get_rect(),  # כל שטח הפאנל
            border_radius=20  # רדיוס הפינות (שנה לפי טעם)
        )

        # צייר מסגרת לבנה בעובי 2px
        pygame.draw.rect(
            panel,
            (255, 255, 255, 120)            ,  # צבע המסגרת (לבן)
            panel.get_rect(),
            width=2,  # עובי המסגרת
            border_radius=20  # חייב להיות אותו רדיוס כדי שתתאים לפינות
        )

        # הצב את הפאנל על המסך
        self.screen.blit(panel, panel.get_rect(center=(cx, mid + 120)))
        ...
        self.screen.blit(dev_surf, dev_surf.get_rect(center=(cx, mid + 148 - 18))) # אחראי על המיקום של ה pc/mobile mode
    ...
    def _draw_difficulty_panel(self):
        ...
        self.screen.blit(title, title.get_rect(center=(cx, self.H//2 + 160 + 25)))  # Moved down to match new button position
