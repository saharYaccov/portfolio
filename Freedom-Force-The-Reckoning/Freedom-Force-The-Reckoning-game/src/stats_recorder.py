...
def calculate_score(
    ...
    extra_kills=0,
    difficulty_bonus=0,
    difficulty_multiplier=1.0,  # ← פרמטר חדש!
):
    """
    ...
    extra_bonus      = extra_kills × 500  (אויבים יורים מיוחדים)

    difficulty_multiplier:
      - easy   → 1.0  (כמו שהוא)
      - middle → 1.6  (כל הניקוד × 1.6)
      - hard   → 2.0  (כל הניקוד × 2)
    """
    stage_score   = int(stage_reached * 1_300 * difficulty_multiplier)
    victory_bonus = int(15_000 * difficulty_multiplier) if outcome == "victory" else 0

    # בוס — הגורם המכריע
    if defeated_boss:
        boss_speed_bonus = max(0, int((10_000 - boss_fight_sec * 50) * difficulty_multiplier))
    else:
        boss_speed_bonus = 0

    minutes      = game_time_sec / 60.0
    speed_bonus  = max(0, int((3_000 * stage_reached / max(0.5, minutes)) * difficulty_multiplier))
    enemy_bonus  = int(enemies_killed * 200 * difficulty_multiplier)

    lasers   = max(1, total_lasers)
    accuracy = enemies_killed / lasers
    accuracy_bonus = int(accuracy * 500 * difficulty_multiplier) if accuracy > 0.3 else 0

    character_bonus = int((kills_by_bb * 25 + kills_by_trump * 50) * difficulty_multiplier)

    extra_bonus = int(extra_kills * 700 * difficulty_multiplier)

    life_penalty = int(lives_lost * 220 * difficulty_multiplier)

    total = (stage_score + victory_bonus + boss_speed_bonus
             + speed_bonus + enemy_bonus + accuracy_bonus
             + character_bonus + extra_bonus + difficulty_bonus
             - life_penalty) - int(5000 * difficulty_multiplier)
    return max(0, total)
...
class StatsRecorder:
    ...
    def reset(self):
        ...
        self._saved              = False
        self._last_score         = 0
        # difficulty (set from game start)
        self.difficulty_label    = "easy"
        self.difficulty_score    = 800
        self.difficulty_multiplier = 1.0  # ← חדש!
        # חדש
        self.kills_by_enemy: dict = {}   # {enemy_name: count}
        self.kills_by_bb    = 0
        self.kills_by_trump = 0
        self.extra_kills    = 0          # אויבים יורים מיוחדים (500 נק' כל אחד)
        self._boss_start_time = None     # מתי התחיל שלב הבוס
        self.boss_fight_sec   = 0.0

    def set_difficulty(self, level: str):
        """
        Set difficulty label + base score according to user selection.
        ...
        if lvl == "easy":
            ...
            label, base = "middle", 2200
        elif lvl == "hard":
            label, base = "hard", 5000
        else:
            label, base = "easy", 800

        self.difficulty_label = label
        self.difficulty_score = base

        # הגדרת מכפיל ניקוד לפי רמת קושי
        if lvl == "easy":
        ...
        print(f"[stats] difficulty set: {label} (base={base}, multiplier={self.difficulty_multiplier}x)")
    ...
    def on_extra_killed(self):    self.extra_kills        += 1
    ...
    def get_score(self, stage_reached, outcome):
        score = calculate_score(
            ...
            extra_kills     = self.extra_kills,
            difficulty_bonus = self.difficulty_score,
            difficulty_multiplier = self.difficulty_multiplier,  # ← הוספה!
        )
