...
- חיבור לאינטרנט
- קובץ המשחק (`game.py`) ותיקיית פרויקט נקייה
...
כדי לוודא ש-Python הותקן בצורה נכונה, הריצו את הפקודה הבאה בטרמינל:
...
💡 **טיפ:** אם המשחק שלכם משתמש במספר ספריות, צרו קובץ בשם `requirements.txt` עם רשימת הספריות (ספריה אחת בכל שורה), לדוגמה:
